...
class PerpHyperliquid:
    ...
    async def get_balance(self) -> UsdcBalance:
        ...
        data = await self._dex.fetch_balance()
    ...
    async def position_infos(self, symbol: str) -> OpenPosition:
        pair = self.symbol_to_pair(symbol)
        # params = {"vaultAddress": self.public_address}
        raw = await self._dex.fetch_position(pair)
    ...
    async def get_position_side(self, symbol: str) -> Optional[str]:
        try:
            # params = {"vaultAddress": self.public_address}
            pair = self.symbol_to_pair(symbol)
            pos = await self._dex.fetch_position(pair)
    ...
    async def cancel_orders(self, symbol: str):
        try:
            ...
            if orders:
                for order in orders:
                    pair = self.symbol_to_pair(symbol)
                    await self._dex.cancel_order(order.id, pair)
    ...
    async def get_order_by_id(self, order_id) -> Order:
        ...
        return Order(
            id=str(order_id),
            pair=self.symbol_to_pair(order["coin"]),
    ...
    async def market_buy(self, symbol: str, amount: float):
        try:
            ...
            order = await self._dex.create_order(pair, 'market', 'buy', amount, price)
    ...
    async def market_sell(self, symbol: str, amount: float):
        try:
            ...
            pair = self.symbol_to_pair(symbol)
            price = await self.get_mid_price(symbol)
            order = await self._dex.create_order(pair, 'market', 'sell', amount, price)
            print(f"Market sell order successfully placed")
            print(order)
        except Exception as e:
            print(f"Error placing market sell order for {symbol}: {e}")

    async def limit_buy(self, symbol: str, amount: float, price: float):
        try:
            # params = {"vaultAddress": self.public_address}
            pair = self.symbol_to_pair(symbol)
            order = await self._dex.create_order(pair, 'limit', 'buy', amount, price)
            order_id = order['id']
    ...
    async def limit_sell(self, symbol: str, amount: float, price: float) -> OpenOrder:
        try:
            # params = {"vaultAddress": self.public_address}
            pair = self.symbol_to_pair(symbol)
            order = await self._dex.create_order(pair, 'limit', 'sell', amount, price)
            order_id = order['id']
            order = await self.get_order_by_id(order_id)
            order.price = float(price)
    ...
    async def market_close(self, symbol: str):
        try:
            ...
            order = await self._dex.create_order(pair, 'market', side, size, price)
    ...
    async def limit_close(self, symbol: str):
        try:
            ...
            side = await self.invert_side(symbol)
            order = await self._dex.create_order(pair, 'limit', side, size, price)
            order_id = order['id']
            order = await self.get_order_by_id(order_id)
            order.price = float(price)
    ...
    async def close_all_positions(self):
        try:
            ...
            for pos in all_pos:
                ...
                await self._dex.create_order(pos_pair, 'market', pos_side, pos_size, close_price)
        except Exception as e:
